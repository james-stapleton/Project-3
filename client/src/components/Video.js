...
export default function Video({name, videoID}) {
    ...
    // if in local storage, just display that. This way each user isn't loading the same drink more than once. However, each new user can load each drink once per day. Not ideal but ok for now. 
    ...
    if ( !localvideoID && localvideoID != '') {
        ...
        //  .then((data) => {
            ...
            // updateVideo({variables: {name: name, videoID: fetchVideoID}});
